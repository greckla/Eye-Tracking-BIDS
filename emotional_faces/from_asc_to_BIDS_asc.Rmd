...
for (file in 1:length(input_file_names)){
  ...
  for (event_nr in 1:length(tmp_file_onset)){
    ...
    ###### OPTIONAL #####
    # readout all potentially relevant information about the stimuli 
    ...
    if (event_nr == 1){
      ...
      blocktrial[event_nr] = info_blocktrial
      #########################################
    }
  }
  ...
  # note, events counter (e.g., trials) is very practical for futher data processing!
  # in case the messages include no counter, you can paste it yourself
  event_df = data.frame(onset,duration,sample,trial, blocktrial,
  ...
  # include the information about the aoi
  event_df$aoi = stim_info$aoi[1:length(event_df$onset)]
  
  event_df = event_df[c("onset", "duration", "sample", "trial_type", "event_nr",
                        "aoi", "trial", "blocktrial")]
  
  event_df$aoiInfo_faces = stim_info$aoiInfo_faces[1:length(event_df$onset)]
  event_df$aoiInfo_emotion = stim_info$aoiInfo_emotion[1:length(event_df$onset)]
  event_df$aoiInfo_model = stim_info$aoiInfo_model[1:length(event_df$onset)]
  event_df$aoiInfo_sex = stim_info$aoiInfo_sex[1:length(event_df$onset)]
  #####################
...
describe_df = read.csv("./descriptives.csv", header = TRUE) 
