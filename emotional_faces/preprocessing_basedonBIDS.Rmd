...
path = "./freeviewfaces_raw_data_BIDS"

# list all subject directories
subjects = list.dirs(path, recursive = T)
subjects = subjects[grepl(pattern="eyetrack", x = subjects)]

# get full file names for *eyetrack.tsv, *.json and *events.tsv files
input_file_pathways = list.files(subjects, pattern = "*eyetrack.asc$",full.names = T)
...
for (input_file in 1:length(input_file_pathways)){
  ...
  rel_columns_dfix = match(c("trial_type","event_nr","trial"),names(dat_fix))
  
  rel_columns_devents = match(c("trial_type","event_nr","trial"),names(dat_events))
  ...
  dat_fix$emotion = NA
  dat_fix$sex = NA
  rel_columns = match(c("model", "emotion", "sex"),names(dat_fix))
  ...
  if(!is.na(dat_events$aoi[event])|dat_events$aoi[event]!=""){
    ...
    model = fromJSON(dat_events$aoiInfo_model[event])
    emotion = fromJSON(dat_events$aoiInfo_emotion[event])
    sex = fromJSON(dat_events$aoiInfo_sex[event])
    
    aoisEvent = data.frame(aoi)
    aoiInfo = data.frame(model, emotion, sex, stringsAsFactors = FALSE)
...
write.csv(dat_fix_cleaned, paste0(outpath, "dat_fix_freeviewfaces.csv"), row.names = FALSE, quote=F)
```
