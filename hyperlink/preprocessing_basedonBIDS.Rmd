...
for (input_file in 1:length(input_file_pathways)){
  ...
  acq = strsplit(strsplit(code, split = "_") [[1]][2], split = "-")[[1]][2]
  ...
  dat_fix$event_nr = NA
  dat_fix$trial = NA
  dat_fix$target_word = NA
  dat_fix$color = NA
  dat_fix$mask = NA
  dat_fix$sentenceNr = NA
  dat_fix$response = NA
  
  # identify the corresponding column numbers
  rel_columns_dfix = match(c("trial_type","event_nr",
          "trial", "target_word", "color",
          "mask", "sentenceNr", "response"),names(dat_fix))
  
  rel_columns_devents = match(c("trial_type","event_nr",
          "trial", "target_word", "color",
          "mask", "sentenceNr", "response"),names(dat_events))
...
# set data types
dat_fix_cleaned$acq = as.factor(dat_fix_cleaned$acq)
...
write.csv(dat_fix_cleaned, paste0(outpath, "dat_fix_hyperlinks.csv"), row.names = FALSE, quote=F)
```
