...
for (file in 1:length(input_file_names)){
  ...
  # read the [file]*.tsv file from the output folder
  ...
  for (event_nr in 1:length(tmp_file_onset)){
    ...
    event_identifier = strsplit(strsplit(tmp_file_onset[event_nr],
                                         split="\t")[[1]][3], split = " ")[[1]][2]
    ###### OPTIONAL #####
  ...
  event_df = data.frame(onset,duration,sample,trial, eventIdentifier,
  ...
  stim_info = stim_info[stim_info$trialID %in% event_df$eventIdentifier,]
  ...
  for(keypress in 1:length(tmp_file_button)){
    ...
    eventIdentifier[keypress] = strsplit(tmp_file_button[[keypress]][4], 
                                         split = "\t")[[1]][2]
    response[keypress] = 
      strsplit(tmp_file_button[[keypress]][4], split = "\t")[[1]][3]
  }
  response_file = data.frame(time, eventIdentifier, response, 
                             stringsAsFactors = FALSE)
  event_df$response[event_df$eventIdentifier%in%response_file$eventIdentifier]=
    response_file$response[response_file$eventIdentifier %in%
