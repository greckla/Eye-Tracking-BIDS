...
dat_efix_all = NULL
input_file_names = list.files("./hyperlinks_raw_data_BIDS/sub-Test/eyetrack/", pattern= "*.asc$")
input_event_names = list.files("./hyperlinks_raw_data_BIDS/sub-Test/eyetrack/", pattern = "*events.tsv")


#loop for all participants
for (nr in 1:length(input_file_names)){
  tmp_file = readLines(paste("./hyperlinks_raw_data_BIDS/sub-Test/eyetrack/", input_file_names[nr],  sep=""))
  code = strsplit(input_file_names[nr], split = "_task-hyperlink_eyetrack")[[1]][1]
  ...
  dat_events=read.table(paste("./hyperlinks_raw_data_BIDS/sub-Test/eyetrack/", input_event_names[nr], sep=""), header=TRUE)
