...
subjects = list.files("./freeviewfaces_raw_data_BIDS/")

for (sub in subjects){
  pathway = paste("./freeviewfaces_raw_data_BIDS/",sub,"/eyetrack/", sep = "")
...
for (nr in 1:length(input_file_pathways)){
  ...
  for (i in c(1:128)){ 
    dat_efix$trial[dat_efix$fix_time_start >= dat_events$start_time[i] & dat_efix$fix_time_start <  (dat_events$start_time[i]+dat_events$duration[i]) |dat_efix$fix_time_end >= dat_events$start_time[i] & dat_efix$fix_time_end<(dat_events$start_time[i]+dat_events$duration[i])] =dat_events$trial[i]}
  ...
  dat_efix$type_fixation[is.na(dat_efix$type_fixation) & !is.na(dat_efix$trial)]="complete"
...
```{r}
hist(dat_fix_cleaned$dur_cor,breaks=1000)

tapply(dat_fix_cleaned$dur_cor,dat_fix_cleaned$eye,summary)
tapply(dat_fix_cleaned$dur_cor,dat_fix_cleaned$emotion,summary)
tapply(dat_fix_cleaned$dur_cor,dat_fix_cleaned$sex,summary)
```
